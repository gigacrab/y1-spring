...
try:
    while True:
        ...
        for i, c in enumerate(cnts):
            ...
            # Compute rect for outer contour once
                    ...
                    if hollow_ratio > 0.85 and extent > 0.85:
                        ...
                        if largest_gc is not None:
                            ...
                            if w_rot == 0 or h_rot == 0:
                                continue
                            aspect_ratio = max(w_rot, h_rot) / min(w_rot, h_rot)
                            print(f"Contained shape selected")
                        elif total_gc_area > MIN_AREA:
                            continue  # likely ORB symbol

                if sel_c is None:
                    if aspect_ratio > MAX_ASPECT_RATIO:
                        continue
                    sel_c = c
                    print(f"No container, using contour {i}")
            ...
            if solidity < 0.70:
                ...
                (_, _), radius = cv2.minEnclosingCircle(sel_c)
